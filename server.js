...
async function identify_user(agent)
{
  ...
  const snap = await client.db("number").collection("contactSchema").findOne({accno: acct_num});
...
function report_issue(agent)
{
  ...
  const intent_val=agent.parameters.isnum;
  ...
  var db2 = db.db("number");
    ...
    db2.collection("compSchema").insertOne(myobj, function(err, res) {
    if (err) throw err;
    db.close();    
  });
 });
...
function welcome(agent) {
  agent.add("Hello there,please enter your account number to proceed");
}
...
intentMap.set("issue", identify_user);
intentMap.set("issue-custom-custom", report_issue);
intentMap.set("issue-custom", custom_payload);
